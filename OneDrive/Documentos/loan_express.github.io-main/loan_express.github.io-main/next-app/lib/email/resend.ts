...
export async function sendWelcomeEmail(email: string, name?: string) {
  try {
    ...
    const userName = escapeHtml(name || email.split('@')[0]);
...
export async function sendContactNotification(data: {
  ...
  try {
    ...
    const escapedAmount = data.amount ? escapeHtml(data.amount) : null;
    const escapedMessage = escapeHtml(data.message);
    
    await resend.emails.send({
      from: FROM_EMAIL,
      to: [ADMIN_EMAIL],
      subject: `Nuevo contacto de ${escapedName}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <div style="background: #f8fafc; padding: 30px; border-radius: 0 0 8px 8px;">
              ...
              <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                <tr style="background: #e2e8f0;">
                  ...
                  <td style="padding: 10px; color: #1e293b;">${escapedName}</td>
                </tr>
                <tr>
                  <td style="padding: 10px; font-weight: bold; color: #475569;">Correo:</td>
                  <td style="padding: 10px; color: #1e293b;"><a href="mailto:${escapedEmail}">${escapedEmail}</a></td>
                </tr>
                ${escapedPhone ? `
                <tr style="background: #e2e8f0;">
                  <td style="padding: 10px; font-weight: bold; color: #475569;">Teléfono:</td>
                  <td style="padding: 10px; color: #1e293b;"><a href="tel:${escapedPhone}">${escapedPhone}</a></td>
                </tr>
                ` : ''}
                ${escapedAmount ? `
                <tr>
                  <td style="padding: 10px; font-weight: bold; color: #475569;">Monto de interés:</td>
                  <td style="padding: 10px; color: #1e293b;">${escapedAmount}</td>
                </tr>
                ` : ''}
              </table>
              ...
              <div style="background: white; padding: 15px; border-left: 4px solid #2563eb; margin: 10px 0;">
                <p style="margin: 0; color: #475569;">${escapedMessage}</p>
              </div>
              
              <div style="margin-top: 30px; text-align: center;">
                <a href="mailto:${escapedEmail}" style="background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">
                  Responder a ${escapedName}
                </a>
              </div>
            </div>
...
export async function sendStatusChangeEmail(data: {
  ...
  try {
    ...
    const escapedName = escapeHtml(data.name);
    const escapedSolicitudNumber = escapeHtml(data.solicitudNumber);
    const escapedAdminNotes = data.adminNotes ? escapeHtml(data.adminNotes) : null;
    ...
    await resend.emails.send({
      from: FROM_EMAIL,
      to: [data.email],
      subject: `Actualización de solicitud ${escapedSolicitudNumber}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
          <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #f8fafc; padding: 30px; border-radius: 0 0 8px 8px;">
              <h2 style="color: #1e3a8a;">Hola, ${escapedName}</h2>
              <p>Tu solicitud <strong>${escapedSolicitudNumber}</strong> ha sido actualizada.</p>
              ...
              <div style="background: #dbeafe; padding: 15px; border-left: 4px solid #2563eb; margin: 20px 0;">
                ...
                <p style="margin: 10px 0 0 0; color: #1e40af;">${escapedAdminNotes}</p>
              </div>
              ` : ''}
